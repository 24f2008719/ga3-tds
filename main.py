...
async def answer_audio(request: Request):
    ...
    prompt = (
        ...
        "fixed permitted set. Triggered by '허용값'/'허용된 값' OR a 'one-of' enumeration.\n"
        "5. correlation MUST be a LIST of objects {\"x\": colA, \"y\": colB, \"type\": "
        "\"positive\"|\"negative\"}. NEVER output a correlation matrix.\n"
        "6. If the transcript states a constraint like '값은 0에서 1 사이입니다', extract "
        "the subject as the column name into 'columns', AND map the constraint in 'explicit_stats'.\n\n"
        f"TRANSCRIPT:\n{transcript}"
    )
